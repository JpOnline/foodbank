...
if(isset($_GET['mode']) && ($_GET['mode'] == 'newvoucher' || $_GET['mode'] == 'viewvoucher')) {
	...
	$editing = false;
		...
		<div><form action='voucher.php' method='post' onsubmit="return validateVoucherForm()">
				...
				<td><h3>Date Voucher Issued</h3></td>
					...
					<tr>
						...
						<td valign='middle'>
						<?PHP if($readonly == '') { ?>
							<h5><select name='client' onchange="getClientAddress(this)" <?PHP echo $readonly; ?>>
						<?PHP }
						...
						<tr>
						    ...
						    <select id='placestype' name='placestype' onchange="getplaces()" <?PHP if(!$exchanged) echo 'disabled'; ?>>
									...
									<option value='dp' <?PHP if($exchanged && $exchangeRows[0]['pointOfIssueType'] == 'dp') echo 'selected=\'selected\'' ?>>Distribution Point</option>
									    <option value='cw' <?PHP if($exchanged && $exchangeRows[0]['pointOfIssueType'] == 'cw') echo 'selected=\'selected\'' ?>>Central Warehouse</option></select><br />
...
<!-- new part -->
						...
						?>
							<tr>
								<?PHP// if($readonly == '') { ?> <!-- isso aqui vai permitir mostrar  somente na area de trocar o voucher e não na visualização do voucher trocado  -->
									...
									<td><input type='text' name='<?PHP echo $FPTRows[$k]['name'] ?>' value='<?PHP if($editing) echo $voucherRow['numberOfAdults']; else echo 0 ?>' maxlength='3' <?PHP echo $readonly; ?>></td>
								<?PHP// } ?>
								<td><?PHP if($readonly == '') { ?>
						...
						<tr>
							...
							<td><textarea name='explanation' id='explanation' value='1' rows='5' style='resize:none; width:100%;' <?PHP echo $readonly; ?>><?PHP if($editing) echo $voucherRow['explanation']; ?></textarea></td>
						</tr>
						</span>
				...
				?>
		<div><input class="form-input-button" type='submit' value='Submit and Print Voucher' id='submitAndPrint' onclick='printIssuedVoucher(<?PHP echo $clientName?>)'></div></form></div>
		<div><input class="form-input-button" type='submit' value='Submit' id='submit'></div></form></div>
