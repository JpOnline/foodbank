...
if(isset($_GET['mode']) && ($_GET['mode'] == 'newvoucher' || $_GET['mode'] == 'viewvoucher')) {
    ...
    $query = $dbh->prepare("SELECT referenceNumber, FoodParcel.id, wasGiven, name FROM FoodParcel, FoodParcelType WHERE idFPType = FoodParcelType.id ORDER BY referenceNumber DESC");
    if($query->execute()) {
...
?>
		...
		<tr>
			...
			<tr><td colspan='2'>&nbsp;</td></tr>
			<tr>
			<td><h3>Nature of Need</h3><?PHP if($readonly == '') { ?><br /><h5>If other, please specify.</h5><?PHP } ?></td>
		<td>
<?PHP
...
if(($readonly != '' && $checked != '') || $readonly == '') {
    echo $non[$i] . '</h5><br />';
			...
			<?PHP if(!$exchanged) { ?>
				...
				<tr>
					<td><h3>Would like to exchange this voucher now?</h3></td>
...
<?PHP for($i = 0; $i < $locationCount; $i++) {
    $selected = ($exchanged && $rowsLocation[$i]['id'] == $exchangeRows[0]['pointOfIssue']) ? 'selected=\'selected\'' : ''; ?>
			...
			<tr><td colspan='2'>&nbsp;</td></tr>
			<tr>
				<td><h3>Reference numbers of Food Parcels given</h3><br /><h5>Ctrl+click to select more than one.</h5></td>
				<td><?PHP if($readonly == '') { ?>
			<select id='foodparcels' name='foodparcels[]' style='width:100%;' multiple='multiple' <?PHP if(!$exchanged) echo 'disabled'; ?>>
					<?PHP } ?>
...
<?PHP }
	...
	?>
	    <div><input class="form-input-button" type='submit' value='Submit and Print Voucher' id='submitAndPrint' onclick='printIssuedVoucher()'></div></form></div>
